...
class PublicController extends Controller
{
    ...
    public function getTag($slug, BlogService $blogService)
    {
        ...
        if (is_plugin_active('meme')) {
            $data = app(\Botble\Meme\Services\MemeService::class)->handleFrontRoutes($slug);
            if (isset($data['slug']) && $data['slug'] !== $slug->key) {
                return redirect()->to(route('public.single', SlugHelper::getPrefix(\Botble\Meme\Models\MemeTag::class) . '/' . $data['slug']));
            }
        } else {

...
abstract class CacheAbstractDecorator implements RepositoryInterface
{
     ...
     * @return mixed
     */
    public function getDataIfExistCache($function, array $args, int $cacheTime = 3600)
    {
        ...
        if (empty($cacheTime)) {
            $cacheTime = (int) setting('cache_time', 60) * 60;
        }
        ...
        try {
            $cacheKey = md5(
                ...
                serialize((json_encode(func_get_args())))
            );
            ...
            $this->cache->put($cacheKey, $cacheData ?? 0, $cacheTime);
